...
| OT-8 | R1 — No append-only audit log, no export, no retention | High | Non-demo data | **Partially mitigated** for LLM/analyst validation failures: `FailureLog` (`src/threatprism/llm/failure_log.py`) is an append-only, hash-chained, tamper-evident sink with `verify()`; wired into `run_backtest` + `run_triage`. Covered by `tests/test_failure_log.py` (chain/tamper/deletion/append-only). **Still open:** case `audit_trail` remains a rewritable SQLite blob (OT-1), and no export/retention. |
