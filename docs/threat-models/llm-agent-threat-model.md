...
**How to address.** Done in code: revision pinned by SHA (a moving tag is rejected by `validate_runtime()`), eval mode, `local_files_only` (no egress), escalate-only merge. Live verification run 2026-05-31: §8.1 evasion-recovery 4/6 RR-L1 **and** §8.4 over-defense **measured at 3/12 (25%)** benign trigger-word SOC strings quarantined — PG2 over-defends on "disregard the previous"/"override the rule"/quoted-attack phrasing it cannot distinguish from analyst notes. Both are regression-guarded by opt-in live tests. The measured FP rate forces an owner design decision (spec 32 §9.1): keep the semantic high band as **quarantine** (max injection defense, availability tax) or demote it to **review** (non-blocking) before any high-volume/auto-close path.

**Severity.** Built, default-off, live-verified: 4/6 RR-L1 recall, **25% FP on trigger-word SOC text**. Post real-LLM: **Medium** (unrecovered paraphrase keeps RR-L1 partially open; the FP-DoS availability risk is now *quantified*, not hypothetical — contained by detector-not-gate but material at volume). Tracked as OT-L11.
