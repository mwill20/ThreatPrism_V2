...
| Third-party dataset onboarding boundary | Sanitized ThreatPrism-native fixtures under `fixtures/curated_datasets/` | Raw third-party dataset rows (gitignored `external_datasets/`) and their self-declared license manifest | In-code license allowlist (manifest cannot self-certify), per-family sanitization (Synthea column-project + tokenize, deepset scoped injection-retention, OTRF fail-closed identifier drop), `sha256` provenance, real-intake replay; `LocalDatasetSource` off-by-default and prod-refused | `DATASET_ALLOWED_LICENSE_REVIEW` / `CuratedDatasetSource` in [demo/seeding.py](../../src/threatprism/demo/seeding.py); adapters under [tools/fixture_factory/adapters/](../../tools/fixture_factory/adapters/) |
| Future retrieval/memory boundary | Approved retrieval corpus and memory records | Untrusted generated summaries and source payloads | **Not implemented yet** — see LLM threat model "Pre-Implementation Requirements"; design in [spec 38](../specs/38_RGOI_LEARNING_LOOP_AND_TRIAGE_CONTEXT.md) (gated; re-open OT-L1/OT-L8 before building) | N/A |
...
## Applied Threat-Model Overlay (OWASP-LLM-style)

This overlay applies the threat model the way the OWASP LLM reference architecture
does: **actors** (users), **processes**, **data stores**, **trust boundaries**, and
**data flow**, with each **threat** drawn as a callout node (blue) pointing — via a
dotted `threatens` edge — at the component or asset it endangers. Threat IDs are
ThreatPrism's own, from the three lens files:
[STRIDE](stride-threat-model.md) (`S/T/R/I/D/E`),
[OWASP-LLM/ATLAS](llm-agent-threat-model.md) (`L01/L02/L09/L13`, `OT-L*`), and
[LINDDUN](healthcare-data-threat-model.md) (`DI*`); `OT-*` are open threats from
[`mitigations-traceability.md`](mitigations-traceability.md).

**Legend:** white = actor · dark = process · blue cylinder = data store ·
blue square = threat · red = untrusted source.
...
> control + test live in [`mitigations-traceability.md`](mitigations-traceability.md).
> The two red (UNTRUSTED) nodes — inbound client/SOAR and LLM output — plus the
> gitignored dataset source are the surfaces every control is built to contain.
