...
| OT-L8 | Memory / write-back unspecified | **Mitigation implemented (demo-only, owner-authorized 2026-06-03)** — governed write-back built per [spec 38](38_RGOI_LEARNING_LOOP_AND_TRIAGE_CONTEXT.md) | Gated trigger satisfied for demo scope | Project owner, 2026-06-03 (supersedes 2026-05-24 Avoid) | `KnowledgeLearningLoop` (`csi/learning_loop.py`): reverse-deny default, Stage-1 tokenization before any write, deterministic human-only promotion (AI cannot self-promote; fail-closed), audit per action. Tests: `tests/test_rgoi_learning_loop.py`. **Not wired to triage** — feeding retrieved cognition to the model (OT-L1) remains Avoid. Production write-back (live data, HTTP write routes, persistence, multi-tenant) still gated per LLM threat model "Before Memory". |
