...
| OT-L11 | L1.1 — Semantic prompt-injection classifier adds model-evasion + false-positive-DoS surface ([spec 32](../specs/32_SEMANTIC_PROMPT_INJECTION_LAYER.md)); now implemented (default-off) with detector-not-gate containment | Medium (post real-LLM) | Effective when enabled with live model | `tests/test_semantic_prompt_firewall.py` (detector-not-gate, review-band, no-egress, threshold-bound covered; live over-defense bound gated) |
