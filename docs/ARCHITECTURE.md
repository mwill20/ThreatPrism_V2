...
Two diagrams generated by the `project-diagrams` skill, grounded in
`src/threatprism/cases/schemas.py` and the transitions in `cases/service.py`.

### Case lifecycle (state machine)

The `CaseStatus` transitions actually performed by the service (`create_case`,
`run_triage`, `submit_feedback`):

```mermaid
stateDiagram-v2
    [*] --> received
    received --> queued_for_triage : create_case
    queued_for_triage --> triage_running : run_triage starts
    triage_running --> triage_completed : passes all guardrails
    triage_running --> needs_analyst_review : guardrail block or provider failure
    triage_completed --> analyst_feedback_submitted : analyst feedback
    needs_analyst_review --> analyst_feedback_submitted : analyst feedback
    triage_completed --> [*]
    needs_analyst_review --> [*]
    analyst_feedback_submitted --> [*]
    note right of triage_running
      Parallel triage_status: queued to running to
      one of completed, blocked_by_guardrail, needs_review.
    end note
```

**Rendered:** [SVG](../assets/diagrams/case-lifecycle.svg) · [PNG](../assets/diagrams/case-lifecycle.png)

> **Honest grounding:** `CaseStatus.normalized`, `.closed`, `.failed` and
> `TriageStatus.failed` are defined enum values but are **not currently set by the
> service** — no transition draws them, so the diagram omits them (the code, as it is,
> not as aspired). Surfacing unused states is a feature of grounding diagrams in code.

### Core data model (entity relationships)

Logical relationships among the case aggregate and its parts. Physically a `CaseRecord`
is persisted as a single JSON blob (`persistence/sqlite.py`); this shows the model, not
the table layout.

```mermaid
erDiagram
    CaseRecord ||--o{ Evidence : contains
    CaseRecord ||--o| TriageReport : produces
    CaseRecord ||--o{ AnalystFeedback : receives
    CaseRecord ||--o{ AuditEvent : logs
    TriageReport ||--o{ Finding : has
    Finding }o--o{ Evidence : "cites via evidence_ids"
    AnalystFeedback ||--o| DisagreementRecord : yields
    CaseRecord {
      string case_id PK
    ...
    AnalystFeedback {
      ...
      string case_id FK
      string report_id FK
      Determination analyst_determination
    }
    DisagreementRecord {
      string case_id FK
      string feedback_id FK
      bool determination_mismatch
    }
    AuditEvent {
...
**Rendered:** [SVG](../assets/diagrams/data-model-er.svg) · [PNG](../assets/diagrams/data-model-er.png)
(regenerate all diagrams: `python tools/render_diagrams.py` then mermaid-cli).
