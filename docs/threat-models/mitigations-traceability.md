...
| OT-8 | R1 — No append-only audit log, no export, no retention | High | Non-demo data | **Largely mitigated:** append-only hash-chained logs cover both LLM/analyst validation failures (`FailureLog`) and the full case audit trail (`HashChainedLog` mirror at `save_case`, opt-in via `AUDIT_LOG_PATH`); operator verify + redaction-safe **export** via `python -m threatprism.persistence.verify_logs --export` (`src/threatprism/persistence/verify_logs.py`). `tests/test_failure_log.py` + `tests/test_audit_log_integrity.py` + `tests/test_verify_logs.py`. **Still open:** automated *retention* policy and the non-audit case payload blob (OT-1). |
...
| OT-L8 | Memory/write-back unspecified | High | Memory implementation | **Mitigated for demo scope (owner-authorized 2026-06-03):** governed write-back built per [spec 38](../specs/38_RGOI_LEARNING_LOOP_AND_TRIAGE_CONTEXT.md) — `KnowledgeLearningLoop` (reverse-deny, Stage-1 tokenize, deterministic human-only promotion, audit). `tests/test_rgoi_learning_loop.py`. **Not wired to triage** (OT-L1 stays Avoid); production write-back (persistence, HTTP routes, multi-tenancy) still gated. |
...
| 2026-06-03 | Project owner (authorized) + Claude | OT-L8 write-back mitigation implemented (demo-only, not wired to triage) | Built the governed RGOI write-back loop per spec 38: `csi/learning_loop.py` (`KnowledgeLearningLoop` — reverse-deny default, Stage-1 tokenization before any write, deterministic human-only promotion with AI fail-closed, audit per action) + `csi/triage_context.py` (approved-tier sanitized cited context builder). The triage-context builder is **not consumed by `run_triage`** — a regression test (`test_triage_pipeline_does_not_consume_rgoi_context`) enforces the disconnection, so OT-L1 (RAG-into-triage) remains Avoid. Spec 21 OT-L8 row converted from Avoid to implemented-Gated-Mitigation. Tests: `tests/test_rgoi_learning_loop.py`, `tests/test_rgoi_triage_context.py`. Production write-back (persistence, HTTP write routes, multi-tenancy, non-demo data) still gated. |
|------|----------|---------|-------|
