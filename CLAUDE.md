...
- `POST /cases/{case_id}/analyst-feedback` — submit analyst feedback (roles `analyst`/`engineer`/`admin`; attribution is server-set from the authenticated identity), compute disagreement metrics
